...
(async () => {
  ...
  try {
    ...
    assert.equal(second.voice_id, "", "incompatible inherited voice is explicitly cleared for a different language");
    ...
    assert.equal(cloud.profile_id, undefined, "local profile is not submitted to a cloud route");
    assert.equal(cloud.target_lang, "ru");
    assert.equal(cloud.voice_id, undefined);
    await open("second");
